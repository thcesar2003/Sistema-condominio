...
def monthly_balances(d, opening_balance):
    ...
    rows = []
    saldo_anterior_excel = float(opening_balance)
    ...
    for p in pd.period_range(first, last, freq="M"):
        ...
        saldo_inicial_mes = saldo_anterior_excel
        ...
        # Busca o último saldo registrado no mês no Excel (saldo total disponível do último dia do mês)
        real = None
        real_date = None
        ...
        saldo_final_mes = real if real is not None else calculado
        ajuste = 0.0 if real is None else (real - calculado)
        ...
        # Atualiza a referência para que o saldo do último dia deste mês seja a base exata do próximo mês
        saldo_anterior_excel = saldo_final_mes
...
with tabs[0]:
    ...
    if not f.empty:
        x = (
            f.assign(Ano_Mes=f["Data_Analise"].dt.to_period("M").astype(str))
        ...
        st.plotly_chart(
            px.line(
                ...
                y="Saldo final",
                markers=True,
                title="Saldo final da conta corrente (Último dia do mês)",
            ),
            use_container_width=True,
        )
    ...
    if saldos.empty:
        ...
        st.dataframe(
            saldos.style.format({
                ...
                if c in saldos.columns
            }),
            use_container_width=True,
            hide_index=True,
        )
        st.info("Regra aplicada: Saldo do último dia do mês no Excel = Saldo Inicial do mês seguinte.")

with tabs[2]:
    st.subheader("Conciliação da conta corrente")
    if col_saldo:
        st.success(f"Coluna de saldo do Excel utilizada: '{col_saldo}'")
    else:
        st.warning("Coluna de saldo não identificada no Excel. Exibindo apenas saldo calculado.")
    st.dataframe(
        saldos.style.format({
            c: money
            for c in [
...
with tabs[3]:
    ...
    st.dataframe(
        fund.style.format({"Fundo no mês": money, "Fundo acumulado": money}),
        use_container_width=True,
        hide_index=True,
    )
    if not fund.empty:
...
with tabs[6]:
    ...
    st.dataframe(
        r.groupby("Categoria")["Valor_Absoluto"]
        .sum()
        .reset_index()
        .sort_values("Valor_Absoluto", ascending=False)
        .style.format({"Valor_Absoluto": money}),
        use_container_width=True,
        hide_index=True,
    )
    st.dataframe(r.sort_values("Data_Analise"), use_container_width=True, hide_index=True)
...
with tabs[10]:
    ...
    if not saldos.empty:
        opt = st.selectbox("Selecione o Mês", saldos["Mês"].tolist())
        row = saldos[saldos["Mês"] == opt].iloc[0]
        dm = df[df["Mes"] == opt]
        buf = io.BytesIO()
        doc = SimpleDocTemplate(
            buf, pagesize=landscape(A4), rightMargin=25, leftMargin=25, topMargin=25, bottomMargin=25
        )
        styles = getSampleStyleSheet()
        story = [
            Paragraph("CONJUNTO RESIDENCIAL JARDIM SAN REMO", styles["Title"]),
            Paragraph(f"Relatório Financeiro Mensal - {opt}", styles["Heading2"]),
            Spacer(1, 10),
        ]

        # Tabela 1: Resumo Bancário (Fechamento e Abertura)
        data = [
            [
            ...
            [
                money(row["Saldo inicial"]),
                money(row["Entradas"]),
                money(row["Despesas"]),
                money(row["Fundo transferido"]),
                money(row["Resultado operacional"]),
                money(row["Saldo final"]),
            ],
        ]
        ...
        story += [
            t,
            Spacer(1, 15),
            Paragraph("Detalhamento por Categoria", styles["Heading2"]),
        ]

        # Tabela 2: Lançamentos / Categorias
        cat = dm.groupby(["Tipo_Calculado", "Categoria"])["Valor_Absoluto"].sum().reset_index()
        rows = [["Tipo", "Categoria", "Valor Total"]] + [
            [str(x.Tipo_Calculado), str(x.Categoria), money(x.Valor_Absoluto)] for x in cat.itertuples()
        ]
        ...
        story += [
            ...
            Spacer(1, 10),
            Paragraph(
                f"Ajuste de Conciliação no mês: {money(row['Ajuste de conciliação'])}", styles["Normal"]
            ),
        ]
        doc.build(story)
        buf.seek(0)
        st.download_button(
            "⬇️ Baixar PDF do Mês", buf.getvalue(), f"Relatorio_San_Remo_{opt}.pdf", "application/pdf"
        )
...
with tabs[11]:
    ...
    st.download_button(
        "⬇️ Baixar CSV",
        f.to_csv(index=False, sep=";", encoding="utf-8-sig"),
        "san_remo_filtrado.csv",
        "text/csv",
    )

st.markdown("---")
st.caption("San Remo • saldo do último dia do mês • relatório PDF corrigido")