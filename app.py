...
def monthly_balances(d, opening_balance):
    ...
    saldo_atual = float(opening_balance)

    prior = d[d["Data_Analise"] < pd.Timestamp(first.start_time)]
    if not prior.empty:
        valid_prior = prior[prior["Saldo_Extrato"].notna()]
        if not valid_prior.empty:
            saldo_atual = float(valid_prior.sort_values(["Data_Analise", "_ordem"]).iloc[-1]["Saldo_Extrato"])
