...
saldo_inicial_manual = st.sidebar.number_input(
    ...
    value=0.0,
    step=100.0,
    help="Utilizado caso o primeiro mês do extrato não contenha a coluna de saldo bancário acumulado."
)
...
def calcular_cronograma_mensal(df_completo, saldo_inicial_start=0.0):
    """
    Calcula a transposição de saldos usando a totalidade dos dados (df_completo)
    para garantir a continuidade histórica dos meses anteriores.
    """
...
# O cálculo do cronograma é feito utilizando a base completa para manter o encadeamento dos saldos
...
with tabs[1]:
    ...
    df_mensal_rel = calcular_cronograma_mensal(df, saldo_inicial_manual)
    ...
    if df_mensal_rel.empty:
        ...
        st.info(
            "📌 **Regra de Transposição**: O **Saldo Final** do mês anterior (ex: 31/08) "
            "é levado automaticamente como **Saldo Inicial** para o mês seguinte (ex: 01/09)."
        )
...
with tabs[2]:
    ...
    df_m = calcular_cronograma_mensal(df, saldo_inicial_manual)
...
with tabs[4]:
    ...
    df_saldo_tab = calcular_cronograma_mensal(df, saldo_inicial_manual)
