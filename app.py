...
saldo_inicial_manual = st.sidebar.number_input(
    ...
    value=0.0,
    step=100.0,
    help="Utilizado caso o extrato não contenha a coluna de saldo bancário acumulado."
)
...
# ============================================================
# LÓGICA DE TRANSPOSIÇÃO DE SALDO E CÁLCULOS
# ============================================================
def resumo_periodo(d):
    ...
    fundo_entradas = d.loc[(d["Categoria"] == NOME_FUNDO) & (d["Tipo_Calculado"] == "Entrada"), "Valor_Absoluto"].sum()
    fundo_saidas = d.loc[(d["Categoria"] == NOME_FUNDO) & (d["Tipo_Calculado"] == "Saída"), "Valor_Absoluto"].sum()
...
def calcular_cronograma_mensal(df_base, saldo_inicial_start=0.0):
    """
    Gera o demonstrativo mensal com a transposição do saldo do último dia
    de cada mês para o primeiro dia do mês subsequente.
    """
    d = df_base[df_base["Data_Analise"].notna()].copy()
    if d.empty:
        return pd.DataFrame()
    ...
    for idx, mes in enumerate(meses_ordenados):
        ...
        # Se existir coluna de saldo no extrato com dados válidos:
        tem_saldo_extrato = "Saldo_Extrato" in grupo.columns and grupo["Saldo_Extrato"].notna().any()

        if tem_saldo_extrato:
            saldo_fim_mes = float(grupo.dropna(subset=["Saldo_Extrato"]).iloc[-1]["Saldo_Extrato"])
            if idx == 0:
                # Tenta estimar o saldo inicial do 1º mês revertendo o fluxo
                saldo_ini_mes = float(grupo.dropna(subset=["Saldo_Extrato"]).iloc[0]["Saldo_Extrato"]) - grupo.iloc[0][col_valor]
            else:
                saldo_ini_mes = saldo_anterior
        else:
            saldo_ini_mes = saldo_anterior
            # Saldo final = Saldo Inicial + Entradas - Despesas - Fundo
            saldo_fim_mes = saldo_ini_mes + r["resultado"] - r["fundo_movimentado"]
        ...
        # TRANSPOSIÇÃO DO SALDO PARA O PRÓXIMO MÊS
        saldo_anterior = saldo_fim_mes

    df_mensal = pd.DataFrame(linhas)
    df_mensal["Fundo Acumulado"] = df_mensal["Fundo no mês"].cumsum()
    return df_mensal
...
df_mensal_calc = calcular_cronograma_mensal(df, saldo_inicial_manual)

saldo_atual_exibicao = df_mensal_calc["Saldo Final"].iloc[-1] if not df_mensal_calc.empty else 0.0
...
with tabs[0]:
    ...
    if df_filtrado.empty:
        ...
        d_dash = df_filtrado[df_filtrado["Data_Analise"].notna()].copy()
        if not d_dash.empty:
            df_mensal_filtrado = calcular_cronograma_mensal(df_filtrado, saldo_inicial_manual)

            comp = df_mensal_filtrado.melt(
                id_vars=["Mês"],
...
with tabs[1]:
    ...
    df_mensal_rel = calcular_cronograma_mensal(df_filtrado, saldo_inicial_manual)
    ...
    if df_mensal_rel.empty:
        ...
        st.info(
            "📌 **Regra de Transposição**: O **Saldo Final** de um mês é o valor existente "
            "no último dia do mês e é levado automaticamente como **Saldo Inicial** do mês seguinte."
        )
...
def gerar_pdf_mensal(df_mes, mes_nome, dados_mes):
    ...
    tabela_resumo = Table(resumo, colWidths=[120] * 6)
...
with tabs[2]:
    ...
    df_m = calcular_cronograma_mensal(df_filtrado, saldo_inicial_manual)
    ...
    if df_m.empty:
        ...
        df_mes_dados = df_filtrado[df_filtrado["Mes"] == mes_sel_pdf]
...
with tabs[4]:
    ...
    df_saldo_tab = calcular_cronograma_mensal(df_filtrado, saldo_inicial_manual)

    if not df_saldo_tab.empty:
        st.dataframe(
            df_saldo_tab[["Mês", "Saldo Inicial", "Resultado Operacional", "Fundo no mês", "Saldo Final"]].style.format({
                "Saldo Inicial": dinheiro,
                "Resultado Operacional": dinheiro,
                "Fundo no mês": dinheiro,
                "Saldo Final": dinheiro,
...
# ============================================================
st.markdown("---")
st.caption("San Remo • Gestão Financeira Condominial | Transposição de saldos ativada.")