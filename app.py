...
def resumo_periodo(d):
    ...
    # Saída da conta corrente aumenta o fundo de reserva; Entrada na conta reduz o fundo
...
def calcular_cronograma_mensal(df_completo, saldo_inicial_start=43285.00, fundo_inicial_start=55881.72):
    """
    Calcula a transposição de saldos preservando a conta corrente e a acumulação
    correta do Fundo de Reserva.
    """
    ...
    for idx, mes in enumerate(meses_ordenados):
        ...
        # O saldo final da conta considera o resultado das operações menos o valor transferido ao fundo
        saldo_fim_mes = saldo_ini_mes + r["resultado"] - r["fundo_movimentado"]

        # O fundo acumulado adiciona o montante depositado
...
with tabs[1]:
    ...
    if df_mensal_rel.empty:
        ...
        st.info(
            "📌 **Correção Aplicada**: As transferências para o Fundo de Reserva agora são somadas ao **Fundo Acumulado** (Aumentando de R$ 55.881,72 em diante) e deduzidas da Conta Corrente."
        )
...
# ============================================================
st.markdown("---")
st.caption("San Remo • Gestão Financeira Condominial | Correção do Fundo de Reserva aplicada.")