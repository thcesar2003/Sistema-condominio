...
def monthly_balances(d, opening_balance):
    ...
    if not prior.empty:
        ...
        if not valid.empty:
            saldo_atual = float(valid.sort_values(["Data_Analise", "_ordem"]).iloc[-1]["Saldo_Extrato"])

    saldo_atual = float(opening_balance) if saldo_atual is None else saldo_atual
    rows = []
    ...
    for p in pd.period_range(first, last, freq="M"):
        ...
        calculado = saldo_atual + movimento
        ...
        saldo_final = real if real is not None else calculado
        ajuste = 0.0 if real is None else (real - calculado)

        rows.append({
            "Mês": str(p),
            "Saldo inicial": float(saldo_atual),
            "Entradas": float(ent),
            "Despesas": float(desp),
...
with tabs[1]:
    ...
    if saldos.empty:
        ...
        st.dataframe(saldos.style.format({c: money for c in ["Saldo inicial", "Entradas", "Despesas", "Fundo transferido", "Resultado operacional", "Movimento conta corrente", "Saldo calculado", "Saldo do extrato", "Ajuste de conciliação", "Saldo final"] if c in saldos.columns}), use_container_width=True, hide_index=True)
        st.info("Regra de conciliação: O Saldo Final do mês N vira o Saldo Inicial do mês N+1.")
...
with tabs[2]:
    ...
    if col_saldo:
        ...
        st.warning("Não foi identificada coluna de saldo no Excel; o sistema mostra apenas o saldo calculated.")
    st.dataframe(saldos.style.format({c: money for c in ["Saldo inicial", "Entradas", "Despesas", "Fundo transferido", "Resultado operacional", "Movimento conta corrente", "Saldo calculated", "Saldo do extrato", "Ajuste de conciliação", "Saldo final"] if c in saldos.columns}), use_container_width=True, hide_index=True)
    if not saldos.empty:
        z = saldos.iloc[-1]
...
with tabs[10]:
    ...
    if not saldos.empty:
        ...
        t.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey), ("GRID", (0, 0), (-1, -1), 0.5, colors.grey), ("ALIGN", (0, 0), (-1, -1), "CENTER")]))
        ...
        t2.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey), ("GRID", (0, 0), (-1, -1), 0.4, colors.grey), ("ALIGN", (2, 1), (2, -1), "RIGHT")]))
