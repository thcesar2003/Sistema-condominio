...
# ============================================================
# CLASSIFICAÇÃO AUTOMÁTICA (REGRA ÚNICA POR LINHA)
# ============================================================
REGRAS = {
...
def classificar_linha(row, colunas_texto, col_valor, col_lanc, col_cnpj, col_razao):
    # Extrai o conteúdo exato de cada campo específico
    ...
    valor_bruto = row.get(col_valor, 0) if col_valor else 0
    tipo_operacao = "Entrada" if valor_bruto > 0 else ("Saída" if valor_bruto < 0 else "Neutro")

    # Texto combinado apenas para a pesquisa secundária de fallback
    texto_combinado = f"{txt_lanc} {txt_razao} {txt_cnpj_bruto}".strip()

    # ------------------------------------------------------------
    # REGRAS ESPECÍFICAS (AVALIADAS UMA ÚNICA VEZ)
    # ------------------------------------------------------------
    
    # 1. Aplicação Privilege INT -> Fundo de reserva do condominio
    if "aplicacao privilege int" in txt_lanc or "privilege int" in txt_lanc:
        return "Fundo de reserva do condominio"

    # 2. CPF/CNPJ 51.877.768/0001-89 -> Mercadinho (pesquisa nos campos CPF/CNPJ, Lançamento ou Razão)
    if "51877768000189" in txt_cnpj_limpo or "51.877.768/0001-89" in txt_cnpj_bruto or "51877768000189" in re.sub(r"\D", "", texto_combinado):
        return "Mercadinho"

    # 3. Valor de entrada de R$ 150,00 -> Salão de festas
    if tipo_operacao == "Entrada" and abs(valor_bruto - 150.0) < 0.01:
        return "Salão de festas"

    # 4. Universal Telecom / Universal Telecom LTDA / Directnet -> Antenas (pesquisa nos campos Lançamento, CNPJ ou Razão)
    termos_antenas = ["universal telecom", "directnet"]
    if any(termo in txt_lanc or termo in txt_razao or termo in texto_combinado for termo in termos_antenas):
        return "Antenas"

    # ------------------------------------------------------------
    # CLASSIFICAÇÃO GERAL POR PALAVRAS-CHAVE
    # ------------------------------------------------------------
    for categoria, palavras in REGRAS.items():
        for palavra in palavras:
            if normalizar_texto(palavra) in texto_combinado:
                return categoria

    return "Outras Despesas" if tipo_operacao == "Saída" else "Receitas"
...
def gerar_pdf_mensal(df_mes, mes_nome, kpis_mes):
    ...
    story.append(Paragraph("<b>Resumo por Categoria</b>", styles['Heading2']))
    cat_summary = (
        df_mes.groupby("Categoria")["Valor_Absoluto"]
        .sum()
        .reset_index()
...
#1BTYPtjBHLLvSXHH_IfKrnqqwVQZ4hh4T"
url = "https://docs.google.com/spreadsheets/d/1DY8-BcxRhWZYPW2rhf07sZBraZk5akTe/edit?usp=sharing&ouid=114733459955895404373&rtpof=true&sd=true"
...
colunas_texto = [
    c for c in [
        ...
        col_razao,
        col_cnpj,
        encontrar_coluna(df, ["descrição", "descricao"]),
...
df["Categoria_Automatica"] = df.apply(
    lambda row: classificar_linha(row, colunas_texto, col_valor, col_lanc, col_cnpj, col_razao),
    axis=1
)
